...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState: pacman.GameState):
        ...
        "*** YOUR CODE HERE ***"
        self.startingGameState=startingGameState #将父类的gamestate记录到子类
        self.HeuristicInfo={}#该字典表示在搜索过程中记录下的有效信息，防止多次查找同一信息，降低效率
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        state=(self.startingPosition,())#一个代表xy坐标，一个代表当前状态下已经访问过的corner的集合
        return state
    ...
    def isGoalState(self, state: Any):
        ...
        """
        "*** YOUR CODE HERE ***"
        return len(state[1])==len(self.corners)#当state[1]中也同时含有4个元素时说明这时已经完成了对四个角落的访问
        util.raiseNotDefined()
            ...
            if not hitsWall:
                ...
                if (nextx,nexty) in self.corners: 
                    if not (nextx,nexty) in state[1]:
                        new_state=((nextx,nexty),(state[1]+((nextx,nexty),)))#(state[1]+((nextx,nexty),))是一个新的元组构造方法，因为元组无法直接加减元素
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    "*** YOUR CODE HERE ***"
    #我们设计启发式函数的思路是，在每次当前节点查看到还未访问过的节点的实际距离，我们去这些距离的最大值
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        "*** YOUR CODE HERE ***"
        #调用UCS一致代价搜索函数，是认为该算法笔记适合寻找任何一个食物的位置，因为它会优先寻找到距离当前位置最近的目标
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        "*** YOUR CODE HERE ***"
        return state in self.food.asList()#因为是找任意一个food的位置，所以只要state在foodlist中，我们就认为它是一个合理的goalstate
        util.raiseNotDefined()
